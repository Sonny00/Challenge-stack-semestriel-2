...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    // Find reserve Moto in cart
    for (let i = 0; i < cart.length; i++) {
      // Find the Moto that the order is reserving
      const reservedMoto = await Moto.find({
        _id: cart[i].productId,
        isReserved: true
      });

      // Find the Moto if it exists in database
      const existedMoto = await Moto.findById(cart[i].productId);

      // If reservedMoto existed, throw an error
      // if (reservedMoto?.length > 0) {
      //   throw new Error(`${cart[i].title} is already reserved`);
      // }

      // If existedMoto DO NOT existed, throw an error
      if (existedMoto == null) {
